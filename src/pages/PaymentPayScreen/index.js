...
export default function PaymentPay({ navigation, route }) {
  ...
  return (
    <View style={styles.PrivacyPolicy} onTouchEnd={() => { if (successModalVisible) navigation.navigate('Store') }}>
      ...
      <View style={{ width: '100%', backgroundColor: "#1455F5", padding: 20, borderRadius: 10, alignSelf: 'center', marginTop: 30 }} onTouchEnd={(e) => { e.stopPropagation(); onClickPay(); }}><Text style={{ color: 'white', alignSelf: 'center', fontSize: 18 }}>Pay ${route.params?.total}</Text></View>
    </View>
  )
}
